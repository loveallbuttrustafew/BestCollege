...
public class StudentController {
    ...
    @PostMapping(value = "/student/upload/donelaboratory", consumes = {"multipart/form-data"})
    public String uploadLaboratory(@RequestParam("file") MultipartFile file, @RequestParam("subjectid") Long subjectId, Model model) {
        try {
            Authentication auth = SecurityContextHolder.getContext().getAuthentication();
            String username = auth.getName();
            storageService.uploadDoneLaboratory(file, userService.findByUsername(username).getId(), subjectId);
            ...
            logger.error("[EXCEPTION] UserNotFound while upload file (" + file.getOriginalFilename() + subjectId + ")");
        }
